...
{	int r = (ratio < (1.0 / SRC_MAX_RATIO) || ratio > (1.0 * SRC_MAX_RATIO)) ;
	if(r) {
		fprintf(stderr, "(ratio(%f) < (1.0 / SRC_MAX_RATIO(%d)) || ratio(%f) > (1.0 * SRC_MAX_RATIO(%d)))\n", ratio, SRC_MAX_RATIO, ratio, SRC_MAX_RATIO);
	}
	return r;
} /* is_bad_src_ratio */
